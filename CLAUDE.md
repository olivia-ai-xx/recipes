...
├── meal-prep-guide.html     ← Current week's meal-prep guide — always overwritten, never versioned. Generated by the /meal-prep skill.
├── scripts/
│   └── build_all_recipes.py ← Regenerates all-recipes.html from recipe_htmls/
├── .claude/skills/meal-prep/SKILL.md ← On-demand weekly meal-prep assistant, invoked via /meal-prep
└── recipe_htmls/
...
- `meal-prep-guide.html` and the weekly grocery list are only ever produced by the user explicitly running `/meal-prep` (see `.claude/skills/meal-prep/SKILL.md`) — never generate or update them as part of the regular add-a-recipe workflow, and never run that skill proactively.
